//PRETTY SURE THIS WORKS CHECK LATER
#include <vector>
#include <iostream>
...
Example:
1 //testcases
10 6 4 //limit , esize , instructionssize
1 2 5 8 20 3 //electronics
3 3 4 2 //instructions
Limit = 10
...
Step 4:
Electronic #2 turned on, power is now at 10.
No overload!
*/
...
bool willOverload(vector<int>& instructions, vector<int>& electronics, int limit) {
    ...
    int checker;
    bool result = false;
    vector<int> twice_checker;
    int twicer;
    for(int x = 0; x < instructions.size(); ++x ){
      twicer = 0;
      checker = instructions.at(x);
      twice_checker.push_back(checker);

      //check for multiple instructions on the same electronic
      for (int i= 0; i < twice_checker.size(); ++i){
          if (twice_checker.at(i) == checker){
            twicer +=1;
          }
      }
      //check for repeat values            (make sure x doesnt cause )
      if (twicer%2 == 0 && twicer != 0){ //&& x <= electronics.size()-2 ){
      total -= electronics.at(x-1);
      }
      else{
      total += electronics.at(x-1);
      }
